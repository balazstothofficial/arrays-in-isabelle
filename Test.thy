...
method frame_try_match = then_else \<open>rule frame_match_pure frame_match frame_match_emp\<close> \<open>frame_norm_assoc\<close> \<open>rule frame_no_match, frame_try_match\<close>
...
method hnr_rule_diff_arr =
  (rule hnr_rule_diff_arr[framed] hnr_copy[framed], frame_inference_2) | rule hnr_return
...
lemma hnr_exE: 
  assumes "\<And>x. hnr (\<Gamma> x) c \<Gamma>' R a"
  shows "hnr (\<exists>\<^sub>Ax. \<Gamma> x) c \<Gamma>' R a"
  using assms norm_pre_ex_rule
  unfolding hnr_def
  by sep_auto
...
method normalize_hnr_pre = simp(no_asm) named_ss HOL_basic_ss_nomatch:
  move_pure_right assn_one_left mult_1_right[where 'a=assn] cong: hnr_pre_cong
...
schematic_goal "hnr (master_assn t * \<up>(t \<turnstile> xs \<sim> xsi)) (?c :: ?'a Heap) ?\<Gamma>' ?R (test1 xs)"
  ...
  apply(hnr_rule_diff_arr)
     apply(hnr_rule_diff_arr)
    (* TODO: Keep pure left, because non-linear use *)
    apply(hnr_rule_diff_arr)
      apply(hnr_rule_diff_arr)
     apply(hnr_rule_diff_arr)
     apply(hnr_rule_diff_arr)
